...
class AIRepairEngine:
    async def execute_repair_pipeline(self, incident_id: int, db: Session) -> dict:
        ...
        try:
            ...
            security_scanner.scan_patch(patch, target=f"{project.name} (Incident #{incident_id})")
            ...
            if github_client.has_token():
                ...
                audit_service.log_event(
                    ...
                    summary=f"Opened Pull Request on {project.name} for Incident #{incident_id}",
                    actor="AI_AGENT",
                    severity="SUCCESS",
                    target=branch_name,
                    details={"pr_url": pr_url, "commit": commit_log}
                )
            else:
                ...
                audit_service.log_event(
                    event_type="AI_REPAIR_COMPLETED_MOCK",
                    summary=f"AI Repair patch created locally for Incident #{incident_id} (Token not set)",
                    actor="AI_AGENT",
                    severity="INFO",
                    target=branch_name
                )
